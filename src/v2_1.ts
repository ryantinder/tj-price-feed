...
export const batchPriceV21 = async (pairs: Pair[]) => {
    const chainid = 43114;
    ...
    const pair_addresses = await Promise.all(
        pairs.map( (pair) : Promise<PairAddress> => {
            ...
            return V2_1_FACTORY.getLBPairInformation(token0, token1, pair.bin).then( ([,addr,,] : [never,string,never,never]) : PairAddress => { return { ...pair, address: utils.getAddress(addr) }})
        })
    )
